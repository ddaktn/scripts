...
then
   ...
   do
      echo ${SERVER}
      echo -e "========================="
      ssh -qo BatchMode=yes -l ${USER} -i /data/${USER}/.ssh/id_rsa -qo StrictHostKeyChecking=no ${SERVER} "sudo systemctl start puppet" ; RC=${?}
      if [ ${RC} -eq 0 ]
      then
...
MESSAGE="\nThe puppet agent was successfully started on the following nodes:\n"
MESSAGE+="${GOOD}"
ERROR="\n\nThe puppet agent was NOT successfully started on the following nodes and needs investigation:\n"
ERROR+="${BAD}"

if [[ ${GOOD} && ${BAD} ]]
then
    echo -e "${MESSAGE}${ERROR}"
    echo -e "${MESSAGE}${ERROR}" | mailx -s "The puppet start on ${COLLECTION} nodes for ${MANIFEST} succeeded WITH ERRORS" ${MailList}
elif [ ${GOOD} ]
then
   echo -e "${GOOD}"
   echo -e "${MESSAGE}" | mailx -s "The puppet start on ${COLLECTION} nodes for ${MANIFEST} succeeded" ${MailList}
elif [ ${BAD} ]
then
   echo -e "${BAD}"
   echo -e "${ERROR}" | mailx -s "The puppet start on ${COLLECTION} nodes for ${MANIFEST} FAILED" ${MailList}
else
